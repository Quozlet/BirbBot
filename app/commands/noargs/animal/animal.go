...
func fetchAnimal(url string) (string, *commands.CommandError) {
	...
	if err != nil {
		log.Println(err)
		return "", commands.NewError("Tried to get an image, but failed to initiate a connection to the API")
	}
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		log.Println(err)
		return "", commands.NewError("Successfully made a connection, but an error occured reading the response")
	}
